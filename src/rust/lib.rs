...
impl Typeahead {
    pub fn new(search_space_string: &str) -> Typeahead {
        let strings : Vec<(String, String)> = search_space_string.split('|').map(|s| (s.to_string(), process_string(s))).collect();
        Typeahead { search_space: strings}
    }

    pub fn search(&self, search_string: &str) -> String {
        let mut some_includes : bool = false;
        let lower_case_search_string = process_string(search_string);
